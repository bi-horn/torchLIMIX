'Adapted from https://github.com/limix/limix-plot/blob/master/limix_plot/_manhattan.py'
...
def get_abs_positions(df):
    """Replicate the abs_pos logic from the manhattan function."""
    data = xr.DataArray(
        data=df["pv"].values,
        dims=["candidate"],
        coords={k: ("candidate", df[k].values) for k in df.columns},
    )
    data["chrom"] = data["chrom"].astype(str)
    data["pos"] = data["pos"].astype(int)
